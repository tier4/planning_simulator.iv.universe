...
// TODO there is no get_object. Was that autogenerated in ROS1?
//#include <npc_simulator/msg/get_object.hpp>
...
private:
  ...
  rclcpp::Service<npc_simulator::GetObject::Request>::SharedPtr getobject_srv_;
  ...
  T waitForParam(const std::string & key)
  {
    ...
    while (rclcpp::ok()) {
      const auto result = getParam<T>(key, value);
      if (result) {
        return value;
      }
